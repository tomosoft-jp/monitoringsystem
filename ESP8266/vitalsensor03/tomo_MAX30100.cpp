...
{
	//Serial.println(F("MAX30100::update"));
    readFifoData();
}
...
{
    if (!readoutsBuffer_isEmpty()) {
        SensorReadout readout = readoutsBuffer_pop();
...
{
	//Serial.println(F("MAX30100::readFifoData"));
...
bool MAX30100::readoutsBuffer_isEmpty()
{
	return count == 0;
}

bool MAX30100::readoutsBuffer_push(SensorReadout value)
{
	if (++tail == readoutsBuffer + RINGBUFFER_SIZE) {
		tail = readoutsBuffer;
	}
	*tail = value;
	if (count == RINGBUFFER_SIZE) {
		if (++head == readoutsBuffer + RINGBUFFER_SIZE) {
			head = readoutsBuffer;
		}
		return false;
	} else {
...
SensorReadout MAX30100::readoutsBuffer_pop()
{
	void(* crash) (void) = 0;
	if (count <= 0) crash();
	SensorReadout result = *tail--;
	if (tail < readoutsBuffer) {
		tail = readoutsBuffer + RINGBUFFER_SIZE - 1;
	}
	count--;
	return result;
}
