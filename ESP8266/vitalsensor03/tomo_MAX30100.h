...
#include "tomo_MAX30100_Registers.h"
...
//    CircularBuffer<SensorReadout, RINGBUFFER_SIZE> readoutsBuffer;
    ...
    bool readoutsBuffer_isEmpty();
	bool readoutsBuffer_push(SensorReadout value) ;
	SensorReadout readoutsBuffer_pop();

    SensorReadout readoutsBuffer[RINGBUFFER_SIZE];
//	T buffer[S];
	SensorReadout *head = readoutsBuffer;
	SensorReadout *tail = readoutsBuffer;
	uint16_t count = 0;
